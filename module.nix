...
}:
{
  ...
  # config.settings.colorscheme = "ayu"; # <- ported from the nixvim config
  # config.settings.colorscheme = "dank"; # <- matugen-generated base16 theme (see colors/dank.lua)
  config.settings.colorscheme = "vesper";
  ...
  config.specs.colorscheme = {
    ...
    data = builtins.getAttr config.settings.colorscheme (
      with pkgs.vimPlugins;
      {
        ...
        "ayu" = neovim-ayu;
        "dank" = base16-nvim;
        # fetched from a flake input (plugins-vesper-nvim), not nixpkgs
        "vesper" = config.nvim-lib.neovimPlugins.vesper-nvim;
      }
    );
  };
